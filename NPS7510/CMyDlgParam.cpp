...
void CMyParamGridCtrl::Mem2Ctr(void)
{
    ...
    aContent.Add(_T("OFF"));
    aContent.Add(_T("ON"));
    AddRowCombox(_T("PT断线检测"), (int)m_pParamNPSR->wToggleAlarmPT, aContent, _T(""));
    AddRowCombox(_T("PT电压VV接线"), (int)m_pParamNPSR->wModePT, aContent, _T(""));

    AddRowEdit(_T("CT变比"), (int)m_pParamNPSR->wScaleCT, _T(""));
    AddRowEdit(_T("PT变比"), (int)m_pParamNPSR->wScalePT, _T(""));
}
