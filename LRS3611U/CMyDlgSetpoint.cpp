...
//     nProIndex++;
	...
	CBCGPGridRow* pRow;
    pRow = AddProtectTog(_T("电流I段"), m_Setpoint.wToggle[0]);
    ...
    pRow = AddProtectTog(_T("电流II段"), m_Setpoint.wToggle[1]);
    ...
    pRow = AddProtectTog(_T("电流III段"), m_Setpoint.wToggle[2]);
    ...
    pRow = AddProtectTog(_T("零序I段"), m_Setpoint.wToggle[3]);
    fData = m_pSetpoint->wLX1_SetI*0.01f;
    AddEditSub(pRow, _T("电流定值"), fData, _T("A"));
    fData = m_pSetpoint->wLX1_SetT*0.01f;
    AddEditSub(pRow, _T("时间定值"), fData, _T("s"));

    pRow = AddProtectTog(_T("零序II段"), m_Setpoint.wToggle[4]);
    fData = m_pSetpoint->wLX2_SetI*0.01f;
    AddEditSub(pRow, _T("电流定值"), fData, _T("A"));
    fData = m_pSetpoint->wLX2_SetT*0.01f;
    AddEditSub(pRow, _T("时间定值"), fData, _T("s"));

	pRow = AddProtectTog(_T("过负荷"), m_Setpoint.wToggle[5]);
    fData = m_pSetpoint->wGFH_SetI*0.01f;
    AddEditSub(pRow, _T("电流定值"), fData, _T("A"));
    fData = m_pSetpoint->wGFH_SetT*0.01f;
    AddEditSub(pRow, _T("时间定值"), fData, _T("s"));


    pRow = AddProtectTog(_T("低电压"), m_Setpoint.wToggle[6]);
    fData = m_pSetpoint->wDDY_SetU*0.01f;
    AddEditSub(pRow, _T("电压定值"), fData, _T("V"));
    fData = m_pSetpoint->wDDY_SetI*0.01f;
    AddEditSub(pRow, _T("闭锁电流"), fData, _T("A"));
    fData = m_pSetpoint->wDDY_SetT*0.01f;
    AddEditSub(pRow, _T("时间定值"), fData, _T("s"));

	pRow = AddProtectTog(_T("重合闸"), m_Setpoint.wToggle[7]);
	fData = m_pSetpoint->wCHZ_SetT*0.01f;
    AddEditSub(pRow, _T("时间定值"), fData, _T("s"));

	pRow = AddProtectTog(_T("后加速"), m_Setpoint.wToggle[8]);
    fData = m_pSetpoint->wHJS_SetI*0.01f;
    AddEditSub(pRow, _T("电流定值"), fData, _T("A"));
    fData = m_pSetpoint->wHJS_SetT*0.01f;
    AddEditSub(pRow, _T("时间定值"), fData, _T("s"));

	AddProtectTog(_T("重瓦斯"), m_Setpoint.INSetpoint[0].wToggle);
    AddProtectTog(_T("轻瓦斯"), m_Setpoint.INSetpoint[1].wToggle);
    AddProtectTog(_T("温高跳闸"), m_Setpoint.INSetpoint[2].wToggle);
    AddProtectTog(_T("温高告警"), m_Setpoint.INSetpoint[3].wToggle);
}
...
//     nProIndex++;
    ...
    m_Setpoint.wToggle[nProIndex++] = GetRowCombox(nPos++);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wDDY_SetU = bcg_round(fData*100);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wDDY_SetI = bcg_round(fData*100);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wDDY_SetT = bcg_round(fData*100);

	m_Setpoint.wToggle[nProIndex++] = GetRowCombox(nPos++);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wGFH_SetI = bcg_round(fData*100);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wGFH_SetT = bcg_round(fData*100);

	m_Setpoint.wToggle[nProIndex++] = GetRowCombox(nPos++);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wCHZ_SetT = bcg_round(fData*100);

	m_Setpoint.wToggle[nProIndex++] = GetRowCombox(nPos++);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wHJS_SetI = bcg_round(fData*100);
    fData = GetRowEditFloat(nPos++);
    m_pSetpoint->wHJS_SetT = bcg_round(fData*100);
}
