...
BOOL CMyUnitModbus::OnAttrib(CBCGPPropertySheet& sheet)
{
    ...
    AfxSetResourceHandle(MYDLL_MODULE);
...
int CMyUnitModbus::MODBUS_Read(COMCONTROL* pCC,WORD wBaseAddress,WORD wCountWord, WORD* pData)
{
    BYTE* pTX = pCCTX;
    *pTX++ = BYTE(m_nAddr);
    *pTX++ = 0x03;
    ...
    *pTX++ = BYTE(wCRC>>8);
    ...
    CopyByte2WORDHL(pData, pRX, wCountWord);
